...
export const convertToTrip = (created: CreatedTrip): Trip => {
    ...
    return {
        ...
        image: (created as any).image || created.images?.[0] || '',
        ...
        tripType: 'trekking',
        spotsRemaining: (created.maxParticipants || (created as any).expectedPorterCount || 20) - (created.participants || 0),
        totalSpots: created.maxParticipants || (created as any).expectedPorterCount || 20,
        leaders: 1,
        portersAvailable: 0,
        portersNeeded: (created as any).expectedPorterCount || 1,
        estimatedPrice: calculateEstimatedPrice(created.includedCosts || []),
        description: created.description || '',
        organizerId: (created as any).creator?.id || (created as any).porter?.id || created.createdBy || '',
    };
};
